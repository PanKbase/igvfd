...
def auth0_access_token():
    creds = {
        ...
        'grant_type': 'password',
        'username': 'test@igvfproject.org',
        'password': 'test'
    }
    url = 'https://pankbase-prod.us.auth0.com/oauth/ro'
    try:
...
@pytest.fixture(scope='session')
def auth0_igvf_user_token(auth0_access_token):
...
def auth0_igvf_user_profile(auth0_access_token):
    user_url = 'https://{domain}/userinfo?access_token={access_token}' \
        .format(domain='pankbase-prod.us.auth0.com', access_token=auth0_access_token)
...
@pytest.fixture(scope='session')
def test_login_no_csrf(anontestapp, auth0_igvf_user_token):
    res = anontestapp.post_json('/login', auth0_igvf_user_token, status=400)
    assert 'Set-Cookie' in res.headers


def test_login_unknown_user(anontestapp, auth0_igvf_user_token):
...
def test_login_logout(testapp, anontestapp, auth0_igvf_user_token, auth0_igvf_user_profile):
    ...
    url = '/users/'
    email = auth0_igvf_user_profile['email']
    item = {
        'email': email,
        'first_name': 'Auth0',
        'last_name': 'Test User',
    ...
    csrf_token = str(res.json['_csrft_'])
    headers = {'X-CSRF-Token': csrf_token}
    res = anontestapp.post_json('/login', auth0_igvf_user_token, headers=headers, status=200)
    assert 'Set-Cookie' in res.headers
    res = anontestapp.get('/session')
